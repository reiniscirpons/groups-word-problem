...
Nielsen's method.~\cite{Lyndon2001aa}

We will not prove this result in this section, rather we will apply some ideas from the proof of
Nielsens method from~\cite[Section 1.2]{Lyndon2001aa}
to concrete subgroups of the free group that appear our chosen proof of
the Boone-Novikov theorem.
...
goal is to prove \cref{thm:subgroup_ap_bq_is_free}.
...
\begin{theorem}\label{thm:subgroup_ap_bq_is_free}
  ...
  $\Gp\genset{a_p, b^q}_{\FG(\Sigma)}$ is free of rank $2$.
  In particular,
  \[
    \Gp\genset{a_{p_1}, b^{q_1}}_{\FG(\Sigma)} \cong
    \Gp\genset{a_{p_2}, b^{q_2}}_{\FG(\Sigma)}
  \]
...
In order to prove this, we will essentially redevelop a small fragment of Nielsen's argument
here, specialized to $\Gp\genset{a_p, b^q}_{\FG(\Sigma)}$.

\reinis{TODO:\\
  1. Small cancellation theory for $a_p, b^q$ \\
  2. Small cancellation product length lemma \\
  3. Coset representative algorithm
}
...
\begin{lemma}\label{lem:HNN_constructive_technical_rws_lemma}
  \uses{def:presentation,def:elementary_sequence}
