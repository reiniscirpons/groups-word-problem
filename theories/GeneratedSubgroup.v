...
Proof.
  ...
  - rewrite morphism_preserve_law /= {1}/(\hat_) /extension /prod /=.
  (* TODO(reiniscirpons): Would be nice if there was some "monoid" or
    ...
    rewrite neutral_right -associativity (H state Hsq).
...
Proof.
  ...
  - rewrite morphism_preserve_law /= {1}/(\hat _) /extension /prod /=.
    rewrite neutral_right -associativity (H 0 H0q).
...
Proof.
  ...
  case HsqP: (state == 0)%B.
  - rewrite morphism_preserve_law /= {1}/(\hat _) /extension /prod /=.
    rewrite neutral_right -associativity (H q.-1 HPqq).
    case: q H0q => [//|m _ /=].
    move/eqP: HsqP => ->; rewrite /b_encoding/=.
    (* NOTE(reiniscirpons): by solve_group_powers *)
